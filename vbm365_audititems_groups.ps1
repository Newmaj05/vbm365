...
$entities = Invoke-RestMethod -Method Get -Uri "$url/Organizations/$organisationId/Groups?limit=10000" -Headers $headers
...
foreach ($group in $groups) {
	$user1 = $entities.results |where-object -property displayName -eq $group
	$auditgroup += $user1
}

$groupDetails = $auditgroup | ForEach-Object {
        @{
            id          = $_.id
